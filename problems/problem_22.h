#ifndef PROBLEM_22
#define PROBLEM_22
#include "problem.h"
#include <fstream>
#include <string>
#include <vector>
#include <algorithm>

/************************
Using names.txt (https://projecteuler.net/project/resources/p022_names.txt),
a 46K text file containing over five-thousand first names,
begin by sorting it into alphabetical order. Then working out the alphabetical value
for each name, multiply this value by its alphabetical position in the list to obtain a name score.

For example, when the list is sorted into alphabetical order, COLIN,
which is worth 3 + 15 + 12 + 9 + 14 = 53, is the 938th name in the list.
So, COLIN would obtain a score of 938 × 53 = 49714.

What is the total of all the name scores in the file?
************************/

namespace problem_22 {

    const constexpr char* FILENAME = "include/p022_names.txt";

    int solve ()
    {
        std::ifstream file(FILENAME);
        std::string name;
        std::vector<std::string> names;

        names.reserve(6000);

        // read from file into names vector
        while (file.good())
        {
            std::getline(file,name,','); // file text is comma separated
            // omitting the wrapping quotes
            names.push_back(name.substr(1, name.length()-2));
        }
        file.close();

        // sorting names
        std::sort (names.begin(), names.end());

        auto i = 0; // name's position in list
        auto x = 0; // current name's value
        auto total = 0; // sum of all name values

        for ( auto n : names )
        {
            x = 0;
            // names in allcaps. 'A' == 65
            for ( auto c : n ) x += ( c - 64 );
            total += ( x * (++i) );
        }

        return total;
    }

    Problem p(22, solve);
}

#endif